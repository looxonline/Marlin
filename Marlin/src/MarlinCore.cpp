 ...
 */
  ...
  void safe_power_minitor() {

    static bool status_known = false;
    static bool last_abnormal = false;
    static float last_vin = -999;

    float vin = safe_power_vin();

    if (ABS(last_vin - vin) < VIN_ERROR_WINDOW) return;
    last_vin = vin;

    bool abnormal = !safe_power_vin_supported(vin);

    if (abnormal) {
      if (!last_abnormal) {
        last_abnormal = true;
        WRITE(SAFE_POWER_PIN, LOW);
      }
    } else if (last_abnormal){
        last_abnormal = false;
    ...
    char buf[64];
    char vin_string[10];
    dtostrf(vin, 1, 2, vin_string);
    sprintf_P(buf, "Power detect %s, Vin = %sV", last_abnormal ? "error" : "ok", vin_string);

    SERIAL_ECHO_START();
    SERIAL_ECHOPGM_P(buf);
    SERIAL_EOL();
  ...
  void safe_power_selfcheck() {
    ...
    OUT_WRITE(SAFE_POWER_PIN, LOW);
      ...
      uint16_t power_stabilization_max = 200;   // Measured in increments of 50ms. 10s total possible wait time.
      uint16_t power_stabilized_count = 0;
      uint16_t power_stabilized_max = 20; // 500ms should be enough to ensure rail stabilization.
      ...
      while (power_stabilization_count++ < power_stabilization_max)
      {
        delay(50);
        ...
        while (safe_power_vin_supported(safe_power_vin()))    // If it has breached the threshold in the last 50ms.
        {
          ...
          if (++power_stabilized_count >= power_stabilized_max)
          {
            power_stabilization_count = power_stabilization_max;    // Force the outer loop to break.
            break;
          } 
        }
      }
    ...
    if (stepper_driver_anti_plug_detect()) {
      failed_status |= 1 << 1;
    }
    if (failed_status) SERIAL_ECHO_MSG("safe power error");
    else {
      WRITE(SAFE_POWER_PIN, HIGH);
...
void idle(TERN_(ADVANCED_PAUSE_FEATURE, bool no_stepper_sleep/*=false*/)) {
  ...
  TERN_(SAFE_POWER, safe_power_minitor());
